...
[[eosio::on_notify("sovmintofeos::transfer")]]
void token::deposit(name from, name to, eosio::asset quantity, std::string memo){
    ...
    check (quantity.amount >= 10000, "must deposit at least 1 SOV");
    asset balance = get_balance( name{"sovmintofeos"}, get_self(), quantity.symbol.code());
   ...
   //rewards distribution code below, no svx rewards for XSOV minting currently 
  ...
  //    svx = svxbalance;
   ...
   action(permission_level{_self, "active"_n}, "xsovxsovxsov"_n, "issue"_n, 
   std::make_tuple(get_self(), safeissue, std::string("Issue XSOV Tokens"))).send();

   action(permission_level{_self, "active"_n}, "xsovxsovxsov"_n, "transfer"_n, 
